...
class App {
  ...
  constructor() {
    ...
    containerWorkouts.addEventListener('click', this._deleteEntry.bind(this));
  ...
  _deleteEntry(e) {
    ...
    if (!deleteBtn) return;
    const entry = deleteBtn.closest('.workout');
    const entryData = this.#workouts.find(work => work.id === entry.dataset.id);
    const entryId = entry.dataset.id;
    const popup = document.querySelector(`.popup-${entryId}`);
    const curMarker = this.#markers[entryId];
    delete this.#markers[entryId];
    this.#map.removeLayer(curMarker);
    if (this.#workouts.length > 0) {
      this.#workouts.splice(this.#workouts.indexOf(entryData), 1);
      entry.remove();
